/* eslint-disable func-style */
/* eslint-disable require-jsdoc */
/* eslint-disable max-len */

/**
 * 添加Blockly块定义，用于 AIbot DHT11 温湿度传感器扩展。
 * @param {Object} Blockly - Blockly核心对象。
 * @returns {Object} Blockly - 返回修改后的Blockly。
 */
function addBlocks(Blockly) {
    const color1 = '#4CAF50';  // 用于语句块（绿色）
    const color2 = '#4C97FF';  // 用于值块（蓝色）

    const dhtIconUrl = 'data:image/svg+xml;base64,';

    const digitalPins = Blockly.getMainWorkspace().getFlyout()
        .getFlyoutItems()
        .find(block => block.type === 'arduino_pin_setDigitalOutput')
        .getField('PIN')
        .getOptions();

    // ===============================
    // 注册 DHT11 IoT 实体
    // ===============================
    Blockly.Blocks['AIbot_register_dht11_sensor_driver_status'] = {
        init: function () {
            this.jsonInit({
                message0: Blockly.Msg.AIBOT_REGISTER_DHT11_SENSOR_DRIVER_STATUS,
                args0: [
                    { type: 'input_value', name: 'TEMP_NAME',         check: 'String' },
                    { type: 'input_value', name: 'TEMP_DESCRIPTION',  check: 'String' },
                    { type: 'input_value', name: 'HUMI_NAME',         check: 'String' },
                    { type: 'input_value', name: 'HUMI_DESCRIPTION',  check: 'String' }
                ],
                colour: color1,
                tooltip: Blockly.Msg.AIBOT_REGISTER_DHT11_SENSOR_DRIVER_STATUS_TOOLTIP || '',
                extensions: ['shape_statement']
            });
        }
    };

    // ===============================
    // 更新 DHT11 IoT 属性值
    // ===============================
    Blockly.Blocks['AIbot_update_dht11_iot_state'] = {
        init: function () {
            this.jsonInit({
                message0: Blockly.Msg.AIBOT_UPDATE_DHT11_IOT_STATE,
                args0: [
                    { type: 'input_value', name: 'PROP_NAME',  check: 'String' },
                    { type: 'input_value', name: 'PROP_VALUE' }
                ],
                colour: color1,
                tooltip: Blockly.Msg.AIBOT_UPDATE_DHT11_IOT_STATE_TOOLTIP || '',
                extensions: ['shape_statement']
            });
        }
    };

    // ===============================
    // DHT 初始化
    // ===============================
    Blockly.Blocks['DHT_init'] = {
        init: function () {
            this.jsonInit({
                message0: '%1',
                message1: Blockly.Msg.DHT_INIT,
                args0: [
                    {
                        type: 'field_image',
                        src: dhtIconUrl,
                        width: 50,
                        height: 27
                    }
                ],
                args1: [
                    {
                        type: 'field_dropdown',
                        name: 'PIN',
                        options: digitalPins
                    },
                    {
                        type: 'field_dropdown',
                        name: 'TYPE',
                        options: [
                            ['DHT11', '11'],
                            ['DHT21', '21'],
                            ['DHT22', '22']
                        ]
                    }
                ],
                colour: color2,
                tooltip: Blockly.Msg.DHT_INIT_TOOLTIP || '',
                extensions: ['shape_statement']
            });
        }
    };

    // ===============================
    // DHT 读取温度或湿度
    // ===============================
    Blockly.Blocks['KE_dht'] = {
        init: function () {
            this.jsonInit({
                message0: '%1',
                message1: Blockly.Msg.KE_DHT,
                args0: [
                    {
                        type: 'field_image',
                        src: dhtIconUrl,
                        width: 50,
                        height: 27
                    }
                ],
                args1: [
                    {
                        type: 'field_dropdown',
                        name: 'MODE',
                        options: [
                            [Blockly.Msg.DHT_TEMPERATURE || 'temperature', 'temperature'],
                            [Blockly.Msg.DHT_HUMIDITY    || 'humidity',    'humidity']
                        ]
                    }
                ],
                colour: color2,
                tooltip: Blockly.Msg.KE_DHT_TOOLTIP || '',
                extensions: ['output_number']
            });
        }
    };

    return Blockly;
}

exports = addBlocks;
