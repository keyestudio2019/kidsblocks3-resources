...
#include "Keyes_IR_ESP32.h"

//public

Keyes_ESP32_IR_Recv::Keyes_ESP32_IR_Recv(uint8_t pin)
{
	//ir_task_handle = NULL;
	ir_rx_pin = (int)pin;
...
Keyes_ESP32_IR_Recv::~Keyes_ESP32_IR_Recv(void)
{	
	end(ir_rx_pin);
}

bool Keyes_ESP32_IR_Recv::begin(uint8_t pin)
{	
	ir_rx_pin = pin;
...
bool Keyes_ESP32_IR_Recv::end(uint8_t pin)
{	
	return rmtDeinit(pin);
}

bool Keyes_ESP32_IR_Recv::available(void)
{
	return ir_recv_data.ir_state;
}
bool Keyes_ESP32_IR_Recv::nec_available(void)
{
...
bool Keyes_ESP32_IR_Recv::sony_available(void)
{
...
bool Keyes_ESP32_IR_Recv::sam_available(void)
{
...
bool Keyes_ESP32_IR_Recv::rc5_available(void)
{
...
const char* Keyes_ESP32_IR_Recv::protocol(void)
{
	return (const char*)proto[ir_recv_data.ir_type].name;
}

uint32_t Keyes_ESP32_IR_Recv::data(void)
{
	ir_recv_data.ir_state = 0;
...
void Keyes_ESP32_IR_Recv::task(void){
...
bool Keyes_ESP32_IR_Recv::check_bit(rmt_data_t &item, uint16_t high, uint16_t low) {
...
uint32_t Keyes_ESP32_IR_Recv::nec_rep_check(rmt_data_t *item, size_t &len) {
...
uint32_t Keyes_ESP32_IR_Recv::nec_check(rmt_data_t *item, size_t &len) {
...
uint32_t Keyes_ESP32_IR_Recv::sam_check(rmt_data_t *item, size_t &len){
	const uint8_t  totalData = 34;
	if(len < totalData ){
		return 0;
	}
...
uint32_t Keyes_ESP32_IR_Recv::sony_check(rmt_data_t *item, size_t &len){
...
uint32_t Keyes_ESP32_IR_Recv::rc5_check(rmt_data_t *item, size_t &len){
	if(len < 13 || len > 30 ){
		return 0;
	}
...
bool Keyes_ESP32_IR_Recv::rc5_bit(uint32_t d, uint32_t v) {
