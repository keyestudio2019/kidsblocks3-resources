// display_lcd.cpp — LCD (ST7789, SPI, 彩色) 版本
// 类名: DisplayLcd
#include <esp_log.h>
...
DisplayLcd::DisplayLcd(esp_lcd_panel_io_handle_t panel_io,
...
void DisplayLcd::SetChatMessage(const Role role, const std::string& content) {
  if (content.empty()) {
    return;
  }
...
void DisplayLcd::ShowStatus(const char* status) {
  lvgl_port_lock(0);
...
void DisplayLcd::SetEmotion(const std::string& emotion) {
