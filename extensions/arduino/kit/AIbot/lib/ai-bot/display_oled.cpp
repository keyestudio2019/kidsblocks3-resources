// display_oled.cpp — OLED (SSD1306, I2C, 单色) 版本
// 类名: DisplayOled
#include "display.h"
...
DisplayOled::DisplayOled(esp_lcd_panel_io_handle_t panel_io, esp_lcd_panel_handle_t panel, int width, int height, bool mirror_x, bool mirror_y) {
  width_ = width;
  height_ = height;
...
void DisplayOled::SetChatMessage(const std::string& content) {
...
void DisplayOled::ShowStatus(const char* status) {
  lvgl_port_lock(0);
...
void DisplayOled::SetEmotion(const std::string& emotion) {
